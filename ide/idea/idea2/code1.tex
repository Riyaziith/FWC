...
\end{center}
 ...
 \begin{center}
  ...
  \textbf{Arduino}& \textbf{2} & \textbf{3}&\textbf{4}&\textbf{5}\\
   \hline
 Inputs&Z&Y&X&F\\ \hline
\end{tabular}   
\end{center}
\caption{\label{table:dummytable} }
\end{table}
...
\\
\\
2. In the another case if we connect the inputs X,Y,Z to 1,1,0 respectively the output should be 0 i.e., the LED connected to 5th pin should turn off
...
The circuit implementation of the above function is given in figure 1.
  ...
  \\
  \begin{lstlisting}
  https://github.com/Riyaziith/FWC
  \end{lstlisting}
